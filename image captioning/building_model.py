...
# use a pre-trained DenseNet121 CNN to capture image features. The last layer for image classification is removed.
model = DenseNet121()
model.layers.pop()
...
start = time()
encoding_train = {}
encoding_train_count = 0
for img in train:
    full_img_path = os.path.join(image_dir, img)
    full_img_path = full_img_path + '.jpg'
    encoding_train[img] = encode(full_img_path)
    encoding_train_count += 1
print("Time taken in seconds =", time()-start)
print(encoding_train_count)
print(encoding_train)

encoded_train_images_dir = os.path.join(root_dir, "encoded_train_images.pkl")

with open(encoded_train_images_dir, "wb") as encoded_pickle:
...
train_features_dir = os.path.join(root_dir, "encoded_train_images.pkl")
test_features_dir = os.path.join(root_dir, "encoded_test_images.pkl")

train_features = load(open(train_features_dir, "rb"))
...
# pass the image features and image captions for model training
...
steps = len(train_descriptions)//number_pics_per_bath # 6000// 3 = 2000
...
final_model.save('model_v2.h5')

print("Finish model training")

final_model.load_weights('model_v2.h5')

#assign z to arbitrary number to pick one of the images for preliminary testing
z = 55
